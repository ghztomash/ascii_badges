def NoiseFloat(x: float, y: float = 0.0, z: float = 0.0) -> float:
    return NoiseInt(int(x * 65536), int(y * 65536), int(z * 65536)) / 65536.0


def NoiseInt(x: int, y: int = 0, z: int = 0) -> int:
    X = (x >> 16) & 255  # xi
    Y = (y >> 16) & 255  # yi
    Z = (z >> 16) & 255  # zi
    N = 1 << 16  # 65536 = 2^16
    ...
    u_t0 = f[x >> 8]
    u_t1 = f[min(255, (x >> 8) + 1)]
    u = u_t0 + (((x & 255) * (u_t1 - u_t0)) >> 8)

    v_t0 = f[y >> 8]
    v_t1 = f[min(255, (y >> 8) + 1)]
    v = v_t0 + (((y & 255) * (v_t1 - v_t0)) >> 8)

    w_t0 = f[z >> 8]
    w_t1 = f[min(255, (z >> 8) + 1)]
    w = w_t0 + (((z & 255) * (w_t1 - w_t0)) >> 8)
    ...
    gradAA = Grad(p[AA], x, y, z)
    gradBA = Grad(p[BA], x - N, y, z)
    lerp_u1 = gradAA + ((u * (gradBA - gradAA)) >> 12)

    gradAB = Grad(p[AB], x, y - N, z)
    gradBB = Grad(p[BB], x - N, y - N, z)
    lerp_u2 = gradAB + ((u * (gradBB - gradAB)) >> 12)

    lerp_v1 = lerp_u1 + ((v * (lerp_u2 - lerp_u1)) >> 12)

    gradAA1 = Grad(p[AA + 1], x, y, z - N)
    gradBA1 = Grad(p[BA + 1], x - N, y, z - N)
    lerp_u3 = gradAA1 + ((u * (gradBA1 - gradAA1)) >> 12)

    gradAB1 = Grad(p[AB + 1], x, y - N, z - N)
    gradBB1 = Grad(p[BB + 1], x - N, y - N, z - N)
    lerp_u4 = gradAB1 + ((u * (gradBB1 - gradAB1)) >> 12)

    lerp_v2 = lerp_u3 + ((v * (lerp_u4 - lerp_u3)) >> 12)

    return lerp_v1 + ((w * (lerp_v2 - lerp_v1)) >> 12)


def Grad(hash: int, x: int, y: int, z: int) -> int:
    h = hash & 15  # CONVERT LO 4 BITS OF HASH CODE INTO 12 GRADIENT DIRECTIONS.
    u = x if h < 8 else y
