...
osc2 = oscillator.Oscillator(oscillator.fps_to_sample_rate(SAMPLE_RATE), 440.0, 2.0 * math.pi / 3.0)
osc3 = oscillator.Oscillator(oscillator.fps_to_sample_rate(SAMPLE_RATE), 440.0, 4.0 * math.pi / 3.0)

center = Vector(WIDTH / 2, HEIGHT / 2)
orbit1 = center + Vector(80, 80)
orbit2 = center + Vector(-80, -80)
orbit3 = center + Vector(80, -80)

chains = []
chains.append(AsciiChain(display, WHITE, PENS_BLUE, center, scale=5, length=1, char_rate=80))
chains.append(AsciiChain(display, WHITE, PENS_BLUE, orbit1, scale=3, length=7, char_rate=150))
chains.append(AsciiChain(display, WHITE, PENS_VIOLET, orbit2, scale=3, length=7, char_rate=175))
chains.append(AsciiChain(display, WHITE, PENS_CYAN, orbit3, scale=3, length=7, char_rate=125))
...
def core1_thread():
    ...
    while True:
        ...
        x2 = int(osc2.get_sin_value() * 80 + center.x)
        y2 = int(osc2.get_cos_value() * 80 + center.y)
        x3 = int(osc3.get_sin_value() * 80 + center.x)
        y3 = int(osc3.get_cos_value() * 80 + center.y)
        lock.acquire()
        chains[1].particles[0].position = Vector(x1, y1)
        chains[2].particles[0].position = Vector(x2, y2)
        chains[3].particles[0].position = Vector(x3, y3)
        lock.release()
